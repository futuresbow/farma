...
class Adminlapgenerator {
	...
	public function urlapGombok($gombok) {
		$ci = getCI();
		$this->tartalom[] = $ci->load->view(ADMINTEMPLATE.'adminlapgenerator/urlapgombok_view', array('gombok' => $gombok), true);
	}
	...
	public function tartalomDobozStart() {
		$ci = getCI();
		$this->tartalom[] = $ci->load->view(ADMINTEMPLATE.'adminlapgenerator/tartalomdobozstart_view', null, true);
	}
	public function tartalomDobozVege() {
		$ci = getCI();
		$this->tartalom[] = $ci->load->view(ADMINTEMPLATE.'adminlapgenerator/tartalomdobozvege_view', null, true);
	}
	public function kimenet() {
		$ci = getCI();
		
		$out = $ci->load->view(ADMINTEMPLATE.'adminlapgenerator/fejlec_view', $this->data, true);
		$out .= $this->tartalomKimenet();
		$out .= $ci->load->view(ADMINTEMPLATE.'adminlapgenerator/lablec_view', $this->data, true);
		return $out;
	}
...
class ALGDoboz {
	...
	public function jodit($selector='#szoveg') {		// WYSWYG editor (Jodit)		$ci = getCI();				$this->ScriptHozzaadas('<link rel="stylesheet" href="//cdnjs.cloudflare.com/ajax/libs/jodit/3.1.39/jodit.min.css">');		$this->ScriptHozzaadas('<script src="//cdnjs.cloudflare.com/ajax/libs/jodit/3.1.39/jodit.min.js"></script>');		$this->ScriptHozzaadas($ci->load->view(ADMINTEMPLATE.'adminlapgenerator/jodit_init', array('selector' => $selector), true));	}
	public function duplaInput($input1 = false, $input2 = false) {
		
		$ci = getCI();
		$this->tartalom[] = $ci->load->view(ADMINTEMPLATE.'adminlapgenerator/duplainput', array('urlapHiba' => $this->data['urlapHiba'],'input1' => $input1, 'input2' => $input2), true);
		
	}
	public function szimplaInput($input1) {
		
		$ci = getCI();
		$this->tartalom[] = $ci->load->view(ADMINTEMPLATE.'adminlapgenerator/szimplainput_view', array('urlapHiba' => $this->data['urlapHiba'],'input1' => $input1), true);
		
	}
	public function dobozElemJelolonegyzetek($jelolok) {
		
		$ci = getCI();
		$this->tartalom[] = $ci->load->view(ADMINTEMPLATE.'adminlapgenerator/boxitemcheckbox_view', array('urlapHiba' => $this->data['urlapHiba'],'jelolok' => $jelolok), true);
		
	}
	public function dobozElem($html) {
		$ci = getCI();
		$this->tartalom[] = $ci->load->view(ADMINTEMPLATE.'adminlapgenerator/boxitem_view', array('html' => $html), true );
		

	}
	public function HTMLHozzaadas($html) {
		$ci = getCI();
		$this->tartalom[] = $ci->load->view(ADMINTEMPLATE.'adminlapgenerator/description_view', array('html' => $html), true );
	...
	public function kimenet() {
		$ci = getCI();
		$out = $ci->load->view(ADMINTEMPLATE.'adminlapgenerator/dobozfejlec_view', $this->data, true);
		$out .= $this->tartalomKimenet();
		$out .= $ci->load->view(ADMINTEMPLATE.'adminlapgenerator/dobozlablec_view', $this->data, true);
		return $out;
	}
	public function tartalomKimenet() {
		$out = '';
...
class ALGTablazat {
	...
	public function kimenet() {
		$ci = getCI();
		$out = $ci->load->view(ADMINTEMPLATE.'adminlapgenerator/tablazat_fejlec_view', $this->data, true);
		$out .= $ci->load->view(ADMINTEMPLATE.'adminlapgenerator/tablazat_sorok_view', $this->data, true);
		
		if($this->lapozo) {			$this->lapozo($this->start,$this->limit,$this->ossz);
			$out .= $ci->load->view(ADMINTEMPLATE.'adminlapgenerator/tablazat_lapozo', $this->data, true);
		}
		if($this->sortable) {
			if(!globalisMemoria('sortableKodBetoltve')) {
				globalisMemoria('sortableKodBetoltve', true);
				$out .= $ci->load->view(ADMINTEMPLATE.'adminlapgenerator/tablazat_sortable', $this->data, true);
			}
			$out .= $ci->load->view(ADMINTEMPLATE.'adminlapgenerator/tablazat_sortable_futtatas', $this->data, true);
		}
		return $out;
	}
...
class Szovegmezo {
	...
	public function kimenet() {
		...
		return $ci->load->view(ADMINTEMPLATE.'adminlapgenerator/input_szoveg_view', $this->data, true);
	} 
} class Urlapgomb {
	var $data = array();	
	public function __construct($adat) { 
	...
	public function kimenet() {
		...
		if($this->data['nevtomb']!='') {
			...
			$this->data['name'] = $this->data['mezonev'];
		}		return $ci->load->view(ADMINTEMPLATE.'adminlapgenerator/input_gomb_view', $this->data, true);
	} 
} 
class AjaxFeltolto {
	var $data = array();
	...
	public function kimenet() {
		...
		return $ci->load->view(ADMINTEMPLATE.'adminlapgenerator/input_ajaxfeltolto', $this->data, true);
	} 
} 
class Filefeltolto {
	var $data = array();
	...
	public function kimenet() {
		...
		return $ci->load->view(ADMINTEMPLATE.'adminlapgenerator/input_file_view', $this->data, true);
	} 
} 
class Szovegdoboz {
	var $data = array();
	...
	public function kimenet() {
		...
		return $ci->load->view(ADMINTEMPLATE.'adminlapgenerator/input_textarea_view', $this->data, true);
	} 
}
class HTMLtartalom {
	var $data = array();
...
class Jelolonegyzet {
	...
	public function kimenet() {
		...
		return $ci->load->view(ADMINTEMPLATE.'adminlapgenerator/input_checkbox_view', $this->data, true);
	} 
} 
class Legordulo {
	var $data = array();
	...
	public function kimenet() {
		...
		return $ci->load->view(ADMINTEMPLATE.'adminlapgenerator/input_select_view', $this->data, true);
	} 
} 
