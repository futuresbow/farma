...
class Felhasznalok_admin extends MY_Modul {
	...
	public function jogkorok () {		globalisMemoria("Nyitott menüpont",'Felhasználók');		globalisMemoria('utvonal', array(array('felirat' => 'Jogkörök')));				if(isset($_POST['jk'])) {			$eleresek =  $this->Sql->sqlSorok("SELECT * FROM  hozzaferesek ORDER BY eleres ASC");						foreach($eleresek as $sor) {				$jogkor = 0;				if(isset($_POST['jk'][$sor->id])) {					foreach($_POST['jk'][$sor->id] as $bw) $jogkor += $bw;				} 								if( ((int)$jogkor & (int)32 )== 0 ) $jogkor += 32; // szuperadminnak minden jár								$this->db->query("UPDATE hozzaferesek SET jogkor = $jogkor WHERE id = {$sor->id} LIMIT 1"); 											}		}				$ALG = new Adminlapgenerator;				$ALG->adatBeallitas('lapCim', "Jogkörök");		$ALG->adatBeallitas('szelessegOsztaly', "full-width");				$ALG->urlapStart(array('attr'=> ' action="" enctype="multipart/form-data" method="post" '));				$ALG->tartalomDobozStart();		$doboz = $ALG->ujDoboz();		$doboz->dobozCim( 'Hozzáférés beállítások', 2);		$szintek = $this->Sql->sqlSorok("SELECT * FROM  felhasznalo_csoportok ORDER BY ertek ASC");		$eleresek =  $this->Sql->sqlSorok("SELECT * FROM  hozzaferesek ORDER BY eleres ASC");								$doboz->HTMLHozzaadas( $this->load->view(ADMINTEMPLATE.'html/jogkor_szerkeszto', array('szintek' => $szintek, 'eleresek' => $eleresek), true) );				$ALG->tartalomDobozVege();		$ALG->urlapGombok(array(						0 => array(				'tipus' => 'submit',				'felirat' => 'Mentés',				'link' => '',				'osztaly' => 'btn-ok',							),		));		$ALG->urlapVege();		return $ALG->kimenet();		}		public function vasarlolista () {
