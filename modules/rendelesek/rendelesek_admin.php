...
class Rendelesek_admin extends MY_Modul {
	...
	public function megrendelesStatuszvaltozasHook($param) {
		...
		if($statusz->keszlet_noveles != 0 or $statusz->foglalt_noveles != 0 ) {
			// készlet módosítás
			$rendeles->megrendelesKeszletmodositas($statusz->keszlet_noveles , $statusz->foglalt_noveles );
	...
	public function statuszlista () {
		globalisMemoria("Nyitott menüpont",'Termékek');
		globalisMemoria('utvonal', array(array('felirat' => 'Státuszok listája')));

		$ALG = new Adminlapgenerator;

		

		$ALG->adatBeallitas('lapCim', "Rendelés státuszok");

		$ALG->adatBeallitas('szelessegOsztaly', "full-width");

		$ALG->adatBeallitas('fejlecGomb', array('url' => ADMINURL.'rendelesek/statuszszerkesztes/0', 'felirat' => 'Új gyártó'));
		...
		$lista = $this->sqlSorok('SELECT * FROM '.DBP.'rendeles_statusz '.$w.' ORDER BY nev ASC ');

		foreach($lista as $sor) {

			

			$sor->alapertelmezett	 = $sor->alapertelmezett	==0?' Nem ':' Igen ';
		...
		$tablazat->adatBeallitas('szerkeszto_url', 'rendelesek/statuszszerkesztes/');

		$tablazat->adatBeallitas('torles_url', 'rendelesek/statusztorles/');

		$tablazat->adatBeallitas('megjelenitettMezok', array('nev' => 'Név', 'kod' => 'Kód','alapertelmezett' => 'Alapértelmezett' ,  'szerkesztes' => 'Szerkesztés',  'torles' => 'Törlés' ));
	...
	public function statuszszerkesztes() {
		globalisMemoria("Nyitott menüpont",'Termékek');
		$ci = getCI();

		$id = (int)$ci->uri->segment(4);

		globalisMemoria('utvonal', array(array('url' => 'rendelesek/statuszlista', 'felirat' => 'Fizetési módok') , array('felirat'=> 'Fizetési mód szerkesztése')));
		...
		if($ci->input->post('a')) {
			...
			if($id == 0) {
				...
				$id = $this->Sql->sqlSave($a, DBP.'rendeles_statusz');
				...
				$this->Sql->sqlUpdate($a,DBP. 'rendeles_statusz');
			...
			redirect(ADMINURL.'rendelesek/statuszlista?m='.urlencode(($p=='')?'Sikeres módosítás.':$p));

		}

		

		$sor = $this->Sql->get($id, DBP.'rendeles_statusz', 'id');
		...
		$ALG->adatBeallitas('lapCim', "Státusz szerkesztése");

		$ALG->adatBeallitas('fejlecGomb', array('url' => ADMINURL.'rendelesek/statuszlista', 'felirat' => 'Rendelés státuszok') );
		...
		$doboz->dobozCim( 'Státusz adatai', 2);

		

		$input1 = new Szovegmezo(array('nevtomb' => 'a', 'mezonev' => 'nev', 'felirat' => 'Megnevezés', 'ertek'=> @$sor->nev));

		$text = new Szovegmezo(array('attr' => " disabled ", 'nevtomb' => 'a', 'mezonev' => 'kod', 'felirat' => 'Kód', 'ertek'=> @$sor->kod));
		...
		$select = new Legordulo(array('attr' => '', 'nevtomb' => 'a', 'mezonev' => 'alapertelmezett', 'felirat' => 'Alapértelmezett', 'ertek'=> @$sor->alapertelmezett, 'opciok' => array(0=>'Nem', 1=>'Igen')));
		$select2 = new Legordulo(array('nevtomb' => 'a', 'mezonev' => 'szamlakeszites', 'felirat' => 'Számlakészítés', 'ertek'=> @$sor->szamlakeszites, 'opciok' => array(0=>'Nem', 1=>'Igen')));
		...
		$ALG->urlapGombok(array(
			...
			0 => array(
				...
				'link' => ADMINURL.'rendelesek/statuszlista',
	...
	public function statusztorles() {
		...
		redirect(ADMINURL.'rendelesek/statuszlista?m='.urlencode('Csak adatbázisból törölhető a státusz!'));
